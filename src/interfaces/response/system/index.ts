import PutCompanyInfoResponseDto from './put-company-info.response.dto';
import GetompanyInfoResponseDto from './get-company-info.response.dto';
import GetDepartmentListResponseDto from './get-department-list.response.dto';
import GetDepartmentInfoResponseDto from './get-department-list.response.dto';
import PutDepartmentInfoResponseDto from './put-department-info.response.dto';
import DeleteDepartmentInfoResponseDto from './delete-department-info.response.dto';

import GetCustomerListResponseDto from './get-customer-list.response.dto';
import PutCustomerInfoResponseDto from './put-customer-info.response.dto';
import DeleteCustomerInfoResponseDto from './delete-customer-info.response.dto';

import GetProductListResponseDto from './get-product-list.response.dto';
import PutProductInfoResponseDto from './put-product-info.response.dto';
import DeleteProductInfoResponseDto from './delete-product-info.response.dto';

export type {
  PutCompanyInfoResponseDto,
  GetompanyInfoResponseDto,
  GetDepartmentListResponseDto,
  PutDepartmentInfoResponseDto,
  DeleteDepartmentInfoResponseDto,
  GetDepartmentInfoResponseDto,
  
  GetCustomerListResponseDto,
  PutCustomerInfoResponseDto,
  DeleteCustomerInfoResponseDto,
  
  GetProductListResponseDto,
  PutProductInfoResponseDto,
  DeleteProductInfoResponseDto,

}